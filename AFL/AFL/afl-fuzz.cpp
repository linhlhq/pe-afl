...
Original WINAFL code written by Ivan Fratric <ifratric@google.com>
pe-afl fork written and maintained by Lucas Leong <lucas_leong@trendmicro.com>
...
static uint8_t simplify_lookup[256] = {
	/*    4 */ 1, 128, 128, 128,
	/*   +4 */ AREP4(128),
	/*   +8 */ AREP8(128),
	/*  +16 */ AREP16(128),
...
static void create_target_process(char** argv) {
	...
	if (!out_file) {
		si.hStdInput = (HANDLE)_get_osfhandle(out_fd);
	}
	else {
		si.hStdInput = hnul;
	}
...
static u8 run_target(char** argv) {
	...
	/* command have (C)rash (R)un (Q)uit O(K) */
	char command[] = "R";
	DWORD num_read;
	char result = 0;
...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault) {
	...
	if (fault == crash_mode) {
		...
		res = calibrate_case(argv, queue_top, (u8*)mem, queue_cycle - 1, 0);
...
static u8 trim_case(char** argv, struct queue_entry* q, u8* in_buf) {
	...
	if (needs_write) {
		...
		if (fd < 0) {
			...
			if (fd < 0) {
				if (errno == EEXIST) {
					fd = _open(q->fname, O_WRONLY | O_CREAT | O_TRUNC | O_BINARY, 0600);
					if (fd < 0) {
...
u8 fuzz_one(char** argv) {
	...
	if (limit_time_sig <= 0) { key_val_lv_1 = fuzz_one_original(argv); }

	if (limit_time_sig != 0) {

		if (key_module == 0) {

			key_val_lv_2 = pilot_fuzzing(argv);

		}
		else if (key_module == 1) {
			key_val_lv_2 = core_fuzzing(argv);
...
static void usage(u8* argv0) {
	...
	SAYF("\n%s [ options ] -- \\path\\to\\fuzzed_app [ ... ]\n\n"
		...
		"  -p path       - path to instrumented PE\n"
...
static void check_crash_handling(void) {
	...
	*BSD, so we can just let it slide for now. */
	...
	s32 fd = _open("/proc/sys/kernel/core_pattern", O_RDONLY);
	u8  fchar;
...
int main(int argc, char** argv) {
	...
	SAYF(cCYA "pe-afl " cBRI VERSION cRST " by <wmliang.tw@gmail.com>\n");
	...
	while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:Qqp:F:L:")) > 0)
		...
		case 'F': /*Power schedule*/
			if (optarg) {
				if (!_stricmp(optarg, "explore")) {
					...
					FATAL("Unknown -F power schedule");
				}
			}
			break;
		case 'L':
